        ...
        while ($statut) 
{
    ...
    Write-Host "[2]" -f $Cyan -NoNewline ; Write-Host " Désactiver le Pare-feu" -f $White;
    Write-Host "[3]" -f $Cyan -NoNewline ; Write-Host " Arrêter le système" -f $White;
    Write-Host "[4]" -f $Cyan -NoNewline ; Write-Host " Redémarrer le système" -f $White;
    ...
    switch ($choix_option) 
	{
	    ...
	    "3" 
	    {
	        $reponse = Read-Host "`nVoulez-vous vraiment arrêter le système ? (oui/non) "
        ...
        "4" 
	    {
	    $reponse = Read-Host "`nVoulez-vous vraiment redémarrer le système ? (oui/non) "
            while ($reponse -ne "non" -and $reponse -ne "n") 
            {
                if ($reponse -eq "oui" -or $reponse -eq "o") 
                {
